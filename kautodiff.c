...
int kad_compare_float(const void *a, const void *b)
{
...
static void kad_print_forward(kad_node_t *p, char* msg) 
{
#ifdef __DRYRUN__
    int rank, size;
    ...
    float *tmp_ary = calloc(kad_len(p), sizeof(float));
    memcpy(tmp_ary, p->x, kad_len(p)*sizeof(float));
    //qsort(tmp_ary, kad_len(p), sizeof(float), kad_compare_float);
    float tmp = 0.0;
    for(int z = 0; z < kad_len(p); z++)
        tmp += tmp_ary[z];
    printf("%s rank: %d %f \n", msg, rank, tmp);
    //printf("%f %f %f %f\n", tmp_ary[0], tmp_ary[kad_len(p)/size-1], tmp_ary[kad_len(p)/size], tmp_ary[kad_len(p)-1]);
#endif
}

static void kad_print_forward2(kad_node_t *p, char* msg) 
{
#ifdef __DRYRUN__
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
static void kad_print_forward_gather(kad_node_t *p, char* msg) 
{
#ifdef __DRYRUN__
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    //MPI_Allgather(p->x, kad_len(p), MPI_FLOAT, global, kad_len(p), MPI_FLOAT, MPI_COMM_WORLD);
    //float tmp = 0.0f;
    //for (int i = 0; i < kad_len(p); ++i)
    //    tmp += p->x[i];
    //printf("rank %d tmp %f\n" ,rank, tmp);
...
static void kad_print_forward_gather2(kad_node_t *p, char* msg) 
{
#ifdef __DRYRUN__
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
static void kad_print_forward_v(float *p, char* msg, int m, int n) 
{
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    ...
    //qsort(p, m*n, sizeof(float), kad_compare_float);
    float tmp = 0.0;
    for(int z = 0; z < m*n; z++)
        tmp += p[z];
    printf("%s rank: %d %f\n", msg, rank, tmp);
...
static void kad_print_forward_gather_v(float *p, char* msg, int m, int n) 
{
#ifdef __DRYRUN__
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    float *global = NULL;
    if (rank == 0) {
        global = calloc(m*n*size, sizeof(float));
    }
    MPI_Gather(p, m*n, MPI_FLOAT, global, m*n, MPI_FLOAT, 0, MPI_COMM_WORLD);
    if (rank == 0) {
        float tmp = 0.0;
        qsort(global, m*n*size, sizeof(float), kad_compare_float);
...
static void kad_print_backward(kad_node_t *p, char* msg) 
{
#ifdef __DRYRUN__
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
static void kad_print_backward_gather(kad_node_t *p, char* msg) 
{
#ifdef __DRYRUN__
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    MPI_Gather(p->g, kad_len(p), MPI_FLOAT, global, kad_len(p), MPI_FLOAT, 0, MPI_COMM_WORLD);
    if (rank == 0) {
        float tmp = 0.0;
        qsort(global, kad_len(p)*size, sizeof(float), kad_compare_float);
        for (int i = 0; i < kad_len(p)*size; i++)
...
static void kad_print_backward2(kad_node_t *p, char* msg) 
{
#ifdef __DRYRUN__
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
static void kad_print_backward_gather2(kad_node_t *p, char* msg) 
{
#ifdef __DRYRUN__
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
KAD_FUNC_OP2(kad_cmul_altsplit, 37)
KAD_FUNC_OP2(kad_cmul_allsplit, 38)
KAD_FUNC_OP2(kad_cmul_altsplit_out, 39)
KAD_FUNC_OP2(kad_cmul_allsplit_out, 40)
...
KAD_FUNC_OP1(kad_fm_allgather, 41)
KAD_FUNC_OP1(kad_fm_allgather2, 42)
KAD_FUNC_OP1(kad_flaten, 43)
...
int kad_op_cmul(kad_node_t *p, int action)
{
    ...
    if (action == KAD_SYNC_DIM) {
        ...
        //printf("m %d n %d k %d\n", n_a_row, n_b_row, n_col);
        kad_print_forward(q[0], "mul_x");
        kad_print_forward_gather(q[1], "mul_w");
        kad_print_forward_gather(p, "mul_p");
    } else if (action == KAD_BACKWARD) {
        if (kad_is_back(q[0]) && q[1]->x) {
            kad_sgemm_simple(0, 0, n_a_row, n_col, n_b_row, p->g, q[1]->x, q[0]->g); /* G_x <- G_y * W */
            //kad_print_backward(p, "cb_mul_gy", n_a_row, n_b_row);
            //kad_print_backward(q[0], "cb_mul_gx", n_a_row, n_col);
            kad_print_backward_gather(q[0], "cb_mul_gx0");//, n_a_row, n_col);
        }
        if (kad_is_back(q[1]) && q[0]->x) {
            kad_sgemm_simple(1, 0, n_b_row, n_col, n_a_row, p->g, q[0]->x, q[1]->g); /* G_w <- trans(G_y) * X */
            //kad_print_backward(q[1], "ug_mul_w", n_b_row, n_col);
            //kad_print_backward_gather(q[1], "ug_mul_w0", n_b_row, n_col);
        }
    }
    return 0;
}

static void kad_allreduce_ext(float *p, int len, int mode)
{
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    ...
    if (mode == INPLACE_REDUCE) {
        MPI_Allreduce(MPI_IN_PLACE, p, len, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
    } 
    else {
        float *global = calloc(len*size, sizeof(float));
        MPI_Allgather(p, len, MPI_FLOAT, global, len, MPI_FLOAT, MPI_COMM_WORLD);
        memset(p, 0, sizeof(float)*len);
        for(int i = 0; i < len; i++) {
...
int kad_op_cmul_alt(kad_node_t *p, int action)
{
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    int i, n_a_row, n_b_row, n_col, n_a_col = 1, n_b_col = 1;
    ...
    if (action == KAD_SYNC_DIM) {
        ...
        kad_allreduce_ext(p->x, kad_len(p), INPLACE_REDUCE);
        kad_print_forward_gather(q[0], "alt_x");
        kad_print_forward_gather(q[1], "alt_w");
        kad_print_forward(p, "alt_p");
    } else if (action == KAD_BACKWARD) {
        if (kad_is_back(q[0]) && q[1]->x) {
            kad_allreduce_ext(p->g, n_a_row*n_b_row, INPLACE_REDUCE);
            kad_sgemm_simple(0, 0, n_a_row, n_col, n_b_row, p->g, q[1]->x, q[0]->g); /* G_x <- G_y * W */
            //kad_print_backward(p, "cb_alt_gy", n_a_row, n_b_row);
            //kad_print_backward(q[0], "cb_alt_gx", n_a_row, n_col);
            kad_print_backward_gather(q[0], "cb_alt_gx0");//, n_a_row, n_col);
        }
        if (kad_is_back(q[1]) && q[0]->x) {
            kad_sgemm_simple(1, 0, n_b_row, n_col, n_a_row, p->g, q[0]->x, q[1]->g); /* G_w <- trans(G_y) * X */
        }
...
int kad_op_cmul_alt_out(kad_node_t *p, int action)
{
    int i, n_a_row, n_b_row, n_col, n_a_col = 1, n_b_col = 1;
    kad_node_t *q[2];
    ...
    if (action == KAD_SYNC_DIM) {
        ...
        kad_allreduce_ext(p->x, kad_len(p), INPLACE_REDUCE);
        kad_print_forward_gather(q[0], "out_x");
        kad_print_forward_gather(q[1], "out_w");
        kad_print_forward(p, "out_p");
    } else if (action == KAD_BACKWARD) {
        if (kad_is_back(q[0]) && q[1]->x) {
            kad_sgemm_simple(0, 0, n_a_row, n_col, n_b_row, p->g, q[1]->x, q[0]->g); /* G_x <- G_y * W */
            //kad_print_backward(q[0], "cb_out_gx", n_a_row, n_col);
            kad_print_backward_gather(q[0], "cb_out_gx0");//, n_a_row, n_col);
        }
        if (kad_is_back(q[1]) && q[0]->x) {
            kad_sgemm_simple(1, 0, n_b_row, n_col, n_a_row, p->g, q[0]->x, q[1]->g); /* G_w <- trans(G_y) * X */
        }
...
int kad_op_cmul_all(kad_node_t *p, int action)
{
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    ...
    int n_col_percore = n_col / size;
    if (action == KAD_SYNC_DIM) {
        p->n_d = 2, p->d[0] = n_a_row, p->d[1] = n_b_row;
    } else if (action == KAD_FORWARD) {
        memset(p->x, 0, n_a_row * n_b_row * sizeof(float));
        if (q[0]->x && q[1]->x) {
            for (int s = 0; s < n_a_row; s++) {
                float *hv = &(q[0]->x[s*n_col_percore]);
                float *hvall = &(p->allval[s*n_col_percore*size]);
                MPI_Allgather(hv, n_col_percore, MPI_FLOAT, hvall, n_col_percore, MPI_FLOAT, MPI_COMM_WORLD);
            }
            kad_sgemm_simple(0, 1, n_a_row, n_b_row, n_col, p->allval, q[1]->x, p->x); 
        }
        //kad_sgemm_simple(0, 1, n_a_row, n_b_row, n_col, q[0]->x, q[1]->x, p->x); /* Y = X * trans(W) */
        //kad_print_forward_gather(q[0], "all_x");
        //kad_print_forward_gather(q[1], "all_w");
        //kad_print_forward_gather(p, "all_p");
    } else if (action == KAD_BACKWARD) {
        if (kad_is_back(q[0]) && q[1]->x) {
            memset(p->allgrad, 0, n_a_row*n_col*sizeof(float));
            kad_sgemm_simple(0, 0, n_a_row, n_col, n_b_row, p->g, q[1]->x, p->allgrad); 
            kad_allreduce_ext(p->allgrad, n_a_row*n_col, INPLACE_REDUCE);
            for (int b = 0; b < n_a_row; b++) {
                memcpy(&(q[0]->g[b*n_col_percore]), &(p->allgrad[b*n_col+rank*n_col_percore]), n_col_percore*sizeof(float));
            }
        }
        //kad_sgemm_simple(0, 0, n_a_row, n_col, n_b_row, p->g, q[1]->x, q[0]->g); /* G_x <- G_y * W */
        //kad_print_backward_gather(p, "cb_all_p", n_a_row, n_b_row);
        //kad_print_backward(q[0], "cb_all_gx", n_a_row, n_col_percore);
        //kad_print_backward_gather(q[0], "cb_all_gx0", n_a_row, n_col_percore);
        if (kad_is_back(q[1]) && q[0]->x)
            kad_sgemm_simple(1, 0, n_b_row, n_col, n_a_row, p->g, p->allval, q[1]->g); 
    }
    return 0;
}

int kad_op_cmul_all_out(kad_node_t *p, int action)
{
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
...
int kad_op_slice(kad_node_t *p, int action)
{
    ...
    if (action == KAD_SYNC_DIM) {
        ...
        for (i = 0; i < d0; ++i) {
            ...
            //float sum = .0;
            //for (int jj = 0; jj < (range[1]-range[0]*d1); jj++)
            //    sum += p->x[i * p->d[axis] * d1 + jj];
            //assert(sum - 1.0 == 0);
        }
    } else if (action == KAD_BACKWARD && kad_is_back(q)) {
        for (i = 0; i < d0; ++i)
...
int kad_op_fm_allgather(kad_node_t *p, int action)
{
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    ...
    if (action == KAD_SYNC_DIM) {
        kad_copy_dim1(p, q);
        p->d[1] *= size; //Concatenate channels
    } else if (action == KAD_FORWARD) {
        for (int b = 0; b < q->d[0]; b++) {
            int skip_q = kad_len(q) / q->d[0];
            int skip_p = kad_len(p) / p->d[0];
            float *send = &(q->x[b*skip_q]);
            float *recv = &(p->x[b*skip_p]);
            MPI_Allgather(send, skip_q, MPI_FLOAT, recv, skip_q, MPI_FLOAT, MPI_COMM_WORLD);
        }
        kad_print_forward(p, "reduce_p_print");
    } else if (action == KAD_BACKWARD && kad_is_back(q)) {
        for (int b = 0; b < q->d[0]; b++) {
            int skip_q = kad_len(q) / q->d[0];
            int skip_p = kad_len(p) / p->d[0];
            float *dst = &(q->g[b*skip_q]);
            float *src = &(p->g[b*skip_p+rank*skip_q]);
            memcpy(dst, src, skip_q*sizeof(float));
        }
        kad_print_backward(p, "reduce_bak_p");
        kad_print_backward(q, "reduce_bak_q");
    }
    return 0;
}

int kad_op_fm_allgather2(kad_node_t *p, int action)
{
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    ...
    if (action == KAD_SYNC_DIM) {
        ...
        int skip_q = kad_len(q) / q->d[0];
        int skip_p = kad_len(p) / p->d[0];
        MPI_Allreduce(MPI_IN_PLACE, p->g, kad_len(p), MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
        for (int b = 0; b < q->d[0]; b++) {
            float *dst = &(q->g[b*skip_q]);
            float *src = &(p->g[b*skip_p+rank*skip_q]);
            memcpy(dst, src, skip_q*sizeof(float));
...
int kad_op_flaten(kad_node_t *p, int action)
{
    int i, n;
    kad_node_t *q = p->child[0];
    if (action == KAD_SYNC_DIM) {
        ...
        memcpy(p->x, q->x, sizeof(float)*kad_len(q));
    } else if (action == KAD_BACKWARD && kad_is_back(q)) {
        memcpy(q->g, p->g, sizeof(float)*kad_len(q));
    }
    return 0;
}
...
#define conv2d_loop2(_x, _w, _y, _code) do { /* for the NHWC shape */ \
    ...
    if (action == KAD_FORWARD || action == KAD_BACKWARD) { /* allocate working space */
        //printf("in rank %d %f %f %f %f\n", rank, q->x[0], q->x[kad_len(q)/q->d[0]-1], q->x[kad_len(q)/q->d[0]], q->x[kad_len(q)-1]);
        //for (int z = 0; z < kad_len(w); z++)
        //    printf("%f ", w->x[z]);
        //printf("\n");
        if (w->d[3] * w->d[1] < 16) {
...
kad_op_f kad_op_list[KAD_MAX_OP] = {
    ...
    kad_op_cmul_alt,   /* 37 */
    kad_op_cmul_all,   /* 38 */
    kad_op_cmul_alt_out,   /* 39 */
    kad_op_cmul_all_out,   /* 40 */
    kad_op_fm_allgather,   /* 41 */
    kad_op_fm_allgather2,   /* 42 */
    kad_op_flaten   /* 43 */
};
...
char *kad_op_name[KAD_MAX_OP] = {
    ...
    "avg1d", "mse", "reshape", "concat", "stdnorm", "exp", "sin", "stack", "reverse", "cmul_alt", "cmul_all", "cmul_alt_out", "cmul_all_out",
    "fm_allgather", "fm_allgather2", "flaten"
};
